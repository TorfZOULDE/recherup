...
const MOTIFS = ["Spam / messages abusifs", "Fausses informations", "Comportement frauduleux", "Autre"];

export default function UtilisateurSuspendModal({ user, onCancel, onConfirm }) {
  const [motif, setMotif] = useState(MOTIFS[0]);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-7 shadow-2xl">
        <h3 className="text-xl font-bold text-gray-900 mb-2">Suspendre ce compte ?</h3>
        <p className="text-sm text-gray-500 mb-5">{user.fullname} ne pourra plus se connecter tant que le compte est suspendu.</p>
